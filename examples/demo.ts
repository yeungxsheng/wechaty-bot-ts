...
async function onMessage (payload: EventMessagePayload) {
  ...
  if (messagePayload.roomId == null) {
     ...
     * 1. 不能是登陆着
     * 2. 发送者不能为空
     * 3. 发送者不在发送集合里面，（目前只做回复1次）
     */
    if (loginerId !== fromId && fromId) {
      if (postMessages.indexOf(fromId) === -1) {
        puppet.messageSendText(fromId, '你好呀~我是小宝，妈妈在睡觉，有急事请电话 - 15018607926').catch(console.error)
        postMessages.push(fromId)
      }
    }
  } else {
